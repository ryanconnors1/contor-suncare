import React from "react";

export default function WhatIsContor() {
  return (
    <section className="mt-16 px-4 sm:px-6 md:px-12 lg:px-24 max-w-3xl mx-auto border-4 border-black rounded-xl p-6 text-center bg-[#F5F3EF]">
      <h2 className="text-2xl font-bold mb-4">What is Contor Shield?</h2>
      <p className="text-gray-800">
        Contor Shield is a unisex SPF 50 body spray that doubles as your signature scent.
        Built for bold protection and premium fragrance, it’s the first sun care product designed to make you smell great — not like sunscreen.
      </p>
    </section>
  );
}